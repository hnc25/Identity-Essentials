...
def display_identity_data():
    ...
    """
    with col1:
        st.write(f"<div style='{box_style}'><b>Total Profiles</b><br>{total_profiles['Total Profiles']:,}</div>", unsafe_allow_html=True)
    with col2:
        st.write(f"<div style='{box_style}'><b>Matched csCoreID</b><br>{total_profiles['Matched csCoreID']:,}</div>", unsafe_allow_html=True)
    with col3:
        st.write(f"<div style='{box_style}'><b>Matched csHHId</b><br>{total_profiles['Matched csHHId']:,}</div>", unsafe_allow_html=True)
    with col4:
        st.write(f"<div style='{box_style}'><b>Duplicate Records (%)</b><br>{total_profiles['Duplicate Records (%)']}%</div>", unsafe_allow_html=True)
...
def main():
    ...
    tab = st.sidebar.radio("Choose a report:", ["Identity", "Hygiene"], index=0)
